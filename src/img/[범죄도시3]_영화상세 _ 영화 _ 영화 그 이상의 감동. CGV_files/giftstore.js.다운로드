...
$.giftstore_fixObj = function( _this, $contents_wrap, $category_content, $giftstore_logo){ // 카테고리 메뉴 & 받은선물/장바구니
    ...
    function hasScrolled(){
        var _target = $(_this);
        var scrollTop = _target.scrollTop() + 60;
        var scrollL = _target.scrollLeft();
        ...
        if(scrollTop >= fixObjTop){
            ...
            $('.category_wrap').css({ 'left': -1 * scrollL })

        }else{
            fixObj.parent().removeClass('fixed_top');

            $('.category_wrap').css({ 'left': 0 })
            /*if(!$category_content.is(':animated')){
            $category_content.stop().animate({
                'padding-left':'0',
                'max-width':'740px'
...
$.fn.asideRePosition = function(){
    ...
    function getReLeftPosition() {
        console.log()
        var left = 980 + ($('footer ul').offset()['left'] + 20);
        return left;
    }
